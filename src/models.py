...
class SystemDecompNet(nn.Module):
    ...
    def __init__(self, weather_dim, nodes, seq_len, hidden_dim=128):
        ...
        self.trend_bias = nn.Parameter(torch.tensor(0.0))
    ...
    def forward(self, x):
        ...
        predicted_fluctuation = self.time_regressor(spatial_out).squeeze(-1) # [seq_len]

        global_trend = trend[:, 0, 0] # [seq_len]
        
        final_out = predicted_fluctuation + (global_trend * self.trend_scale + self.trend_bias)
